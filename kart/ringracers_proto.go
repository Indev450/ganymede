/*
	Ring Racers protocol implementation
...
type RingracersProtocol struct {}

func ringracersReadServerInfo(packet io.Reader, info *KartServerInfo) bool {
...
func ringracersReadPlayerInfo(packet io.Reader, info *KartServerInfo) bool {
	var plrinfo [32]struct {
		Node uint8
		Name [21+1]byte
		Address [4]byte
		Team uint8
		DeprecatedSkin uint8
		Data uint8
		Score uint32
		Timeinserver uint16
	}
...
func (p RingracersProtocol) UpdateServerInfo(packet []byte, info *KartServerInfo) bool {
	var doomdata struct {
		Checksum uint32
		_ uint8
		_ uint8
	...
	if doomdata.Packettype == 13 { // PT_SERVERINFO
		return ringracersReadServerInfo(reader, info)
	} else if doomdata.Packettype == 14 { // PT_PLAYERINFO
		return ringracersReadPlayerInfo(reader, info)
	}

	return false
}

func (p RingracersProtocol) AskServerInfo() []byte {
	data := []byte{
		0, 0, 0, 0, // checksum
		0, 0, // padding
